...
## Usage:
The interpreter can be used in one of two ways, either by running code from a file, or by using an interactive environment to write and run code on the fly.
...
This will run the file and exit upon completion. The `tests` directory has two example you try this with.
...
- `:r` resets the pointer, setting the pointer index to `0`
